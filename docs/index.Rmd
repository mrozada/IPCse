...
**Durante `r meses[mes_t,1]`, el comportamiento del nivel general del IPCse fue parejo en casi todas las regiones; en todas se registró una medición oficial que fue mayor que la medición sin estacionalidad, con diferencias entre los 0.2 y 1.1 puntos porcentuales.** La región GBA  fue la región con mayor diferencia, con 1.1 puntos porcentuales, seguida de las regiones Pampeana, Noroeste y Cuyo, con 0.8 puntos porcentuales de diferencia (Figura 1).<br>

```{r, include=FALSE}
fg1t_wide %>%  arrange(desc(Diferencia))
```

**El índice de precios de Educación fue, con diferencia, el que más aumentó en el mes, con un incremento de `r cuadro1[18,7]` por ciento, seguido del índice de Comunicación, cuyo incremento fue de `r cuadro1[16,7]`.** En un segundo escalón, los índices de Vivienda, agua, electricidad, gas y otros combustibles, Transporte y Salud tuvieron aumentos de `r cuadro1[12,7]`, `r cuadro1[15,7]` y `r cuadro1[14,7]` por ciento respectivamente (Cuadro 1).<br>

```{r, include=FALSE}
fg2_wide %>%  arrange(desc(IPCse))
```

**El índice de precios de Equipamiento y mantenimiento del hogar fue el que menos aumentó en la medición sin estacionalidad, con una suba de `r cuadro1[13,7]` por ciento,**  seguido del índice de precios de Prendas de vestir y calzado, cuyo aumento en la medición sin estacionalidad fue de `r cuadro1[11,7]`  (Figura 2).<br>

```{r cuadrito_incidencias_mensual, include=FALSE}
arrange(cuadro2[5:16,],desc(Nacional))
```

**La contribución más importante al dato del nivel general del IPCse de `r meses[mes_t,1]` la hizo el índice de precios de la división Alimentos y bebidas no alcohólicas explicando alrededor de `r round(100*cuadro2[5,8]/cuadro2[2,8],0)` por ciento del `r cuadro1[2,7]` por ciento de inflación.**  El índice de precios de la división Alimentos y bebidas no alcohólicas contribuyó a explicar más de un 33 por ciento de la inflación sin estacionalidad en las regiones Cuyo y Noroeste, mientras que en la región del Gran Buenos Aires la división de Alimentos y bebidas no alcohólicas explicó el 28.5 por ciento del incremento en el IPCse regional. Durante marzo la división de Transporte volvió a incidir considerablemente en el dato de inflación, explicando un 15 por ciento de la cifra para todo el país. El efecto de esta división es dispar entre regiones, explicando 26 por ciento de la cifra de inflación para la región Noreste, pero entre 14 y 19 por ciento para el resto de las regiones. También fue considerable el aumento explicado por las divisiones de Salud, explicando un 11 por ciento de la tasa de inflación sin estacionalidad a nivel nacional.  (Cuadro 2).<br>
...
```{r cuadro1, echo=FALSE, message=FALSE}
  ...
  cuadro1 %>%   
    ...
    kable_styling() %>%
    add_header_above(c("Año"=1, "2023"=3, "2024"=3, setNames(1,text_mes_yoy))) %>%
...
La Figura 3 muestra la diferencia entre la inflación anual experimentada por el 20 por ciento de la población de menores ingresos y el 20 por ciento de mayores ingresos (medido por el ingreso familiar per capita del hogar). Durante el mes de `r meses[mes_t,1]`, el comportamiento de las tasas de inflación interanuales experimentadas por los hogares de mayores y menores ingresos profundizó el cambio iniciado en el mes anterior. **En particular, durante `r meses[mes_t,1]`, la brecha se ubicó en 3.0 puntos porcentuales en favor de los hogares de menores ingresos.** Este comportamiento se explica por el cambio abrupto en las tasas de inflación interanuales de las divisiones en las que los hogares de mayores ingresos consumen más, como Transporte, Bienes y servicios varios, Vivienda, agua, electricidad, gas y otros combustibles y Salud, y particularmente en este mes, en Educación, contra los cambios no tan grandes en la tasa de inflación interanual de Alimentos y bebidas no alcohólicas y Prendas de vestir y calzado.
