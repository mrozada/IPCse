...
En `r meses[mes_t,1]` los factores estacionales contribuyeron a que la medición oficial de la inflación fuera `r text1` que sin estacionalidad, `r fg3t[1,2]` por ciento versus `r fg3t[1,3]` por ciento. Esta diferencia de `r abs(dif_indec_ipcse)` puntos porcentuales en la inflación con estacionalidad de `r meses[mes_t,1]` se explica fundamentalmente por el comportamiento de las divisiones de Educación, Prendas de vestir y calzado y Alimentos y bebidas no alcohólicas, donde los factores estacionales hicieron que la medición oficial fuera entre 0.9 y 1 punto porcentuales más baja (Figura 2). Durante los últimos 12 meses, el índice aumentó un `r cuadro1[2,8]` por ciento antes del ajuste estacional (datos oficiales de INDEC). <br>

```{r, include=FALSE}
fg2_wide %>%  arrange(desc(Diferencia))
```

**Durante `r meses[mes_t,1]`, todas las regiones registraron una tasa de inflación oficial menor a la medición sin estacionalidad, con diferencias entre 0.3 y 0.5 puntos porcentuales.** La región del Gran Buenos Aires registró la mayor diferencia, con una tasa de inflación oficial que fue 0.5 puntos porcentuales menor, meintras que las regiones Pampeana, Noroeste y Cuyo obtuvieron mediciones 0.3 puntos más bajas que la medición sin estacionalidad. (Figura 1).<br>

```{r, include=FALSE}
fg1t_wide %>%  arrange(desc(Diferencia))
```

**El índice de precios de Comunicación fue el que más aumentó en el mes, con un incremento de `r cuadro1[16,7]` por ciento, seguido del índice de Educación, cuyo incremento fue de `r cuadro1[18,7]`.** Destacan también los aumentos en las mediciones sin estacionalidad de los índices de Bebidas alcohólicas y tabaco, con un aumento de `r cuadro1[10,7]` y de Alimentos y bebidas no alcohólicas, con `r cuadro1[9,7]`  (Cuadro 1).<br>

```{r, include=FALSE}
fg2_wide %>%  arrange(desc(IPCse))
```

**El índice de precios de Salud fue el que menos aumentó en la medición sin estacionalidad, con una suba de `r cuadro1[14,7]` por ciento,**  seguido del índice de precios de Vivienda, agua, electricidad, gas y otros combustibles, cuyo aumento en la medición sin estacionalidad fue de `r cuadro1[12,7]`  (Figura 2).<br>

```{r cuadrito_incidencias_mensual, include=FALSE}
arrange(cuadro2[5:16,],desc(Nacional))
```

**La contribución más importante al dato del nivel general del IPCse de `r meses[mes_t,1]` la hizo el índice de precios de la división Alimentos y bebidas no alcohólicas explicando alrededor de `r round(100*cuadro2[5,8]/cuadro2[2,8],0)` por ciento del `r cuadro1[2,7]` por ciento de inflación.**  El índice de precios de la división Vivienda, agua, electricidad, gas y otros combustibles contribuyó a explicar un 35.7 por ciento de la inflación sin estacionalidad en la región GBA. Esta división explicó 45.7 por ciento en la región Noroeste, y 30 por ciento en la región Patagonia. Durante `r meses[mes_t,1]` la división de Restaurantes y hoteles tomó el segundo lugar en la incidencia, explicando un 11.2 por ciento de la cifra para todo el país. Representó un 13.3 por ciento de la inflación en la región GBA, alrededor de un 10 por ciento en las regiones Pampeana y Noroeste, pero obtuvo un peso considerablemente menor en las regiones Noreste, Cuyo y Patagonia, con incidencias entre el 3 y 7 por ciento. En esas regiones, la división con la mayor incidencia después de Alimentos y bebidas no alcohólicas fue la división de Transporte, representando entre el 14 y 23 puntos porcentuales de la cifra de inflación. (Cuadro 2).<br>
...
La Figura 3 muestra la diferencia entre la inflación anual experimentada por el 20 por ciento de la población de menores ingresos y el 20 por ciento de mayores ingresos (medido por el ingreso familiar per capita del hogar). Durante el mes de `r meses[mes_t,1]`, el comportamiento de las tasas de inflación interanuales experimentadas por los hogares de mayores y menores ingresos mantuvo el mismo signo que en los últimos dos meses. **Durante `r meses[mes_t,1]`, la brecha se ubicó en 3.6 puntos porcentuales en favor de los hogares de menores ingresos.** Este comportamiento se explica por los valores aún elevados de las tasas de inflación interanuales de las divisiones en las que los hogares de mayores ingresos consumen más, como Transporte, Bienes y servicios varios, Vivienda, agua, electricidad, gas y otros combustibles y Salud, contra los cambios no tan grandes en la tasa de inflación interanual de Alimentos y bebidas no alcohólicas y Prendas de vestir y calzado.
...
El Cuadro 4 muestra el impacto diferencial de la inflación por quintiles de ingreso per cápita familiar para los datos oficiales. **Los hogares de menores ingresos, quintil 1, enfrentaron en marzo una inflación de `r sprintf("%.1f",cuadro4[1,3])` por ciento comparada con una inflación de `r sprintf("%.1f",cuadro4[5,3])` por ciento para los hogares del 20 por ciento de mayores ingresos.** Los hogares del quintil 1 enfrentaron una inflación de `r sprintf("%.2f",cuadro4[1,3])` por ciento, comparada con una inflación de `r sprintf("%.2f",cuadro4[5,3])` por ciento para el quintil 5 (ver Figura 5). Esta diferencia se debe al aumento de los precios de las divisiones de Alimentos y bebidas no alcohólicas y Prendas de vestir y calzado, que impactan en mayor medida en los hogares de menores ingresos (quintil 1), que no llegan a ser contrarrestados por los aumentos relativos de los precios de las divisiones de Restaurantes y hoteles, Transporte y Educación, que impactan más en los hogares de mayores ingresos (quintil 5).
