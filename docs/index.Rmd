...
**Durante `r meses[mes_t,1]`, todas las regiones, excepto GBA y Patagonia, registraron una tasa de inflación oficial menor a la medición sin estacionalidad, con diferencias entre 0.1 y 0.3 puntos porcentuales.** En la región del GBA no hubo diferencians entre la inflación sin y con estacionalidad y en Patagonia la tasa de inflación oficial que fue 0.1 puntos porcentuales más alta que la medición sin estacionalidad. (Figura 1).<br>

```{r, include=FALSE}
fg1t_wide %>%  arrange(desc(Diferencia))
```

**El índice de precios de Bebidas alcohólicas y tabaco fue el que, con diferencia, más aumentó en el mes, con un incremento de `r cuadro1[10,7]`. En segundo lugar en los aumentos está el índice de precios sin estacionalidad de Restaurantes y hoteles cuyo incremento fue de `r cuadro1[19,7]`.** Siguen los aumentos en las mediciones sin estacionalidad de los índices de Vivienda, electricidad, gas, agua y otros combustibles y Salud, con aumentos de `r cuadro1[12,7]` y de `r cuadro1[14,7]`  (Cuadro 1).<br>

```{r, include=FALSE}
fg2_wide %>%  arrange(desc(IPCse))
```

**El índice de precios de Prendas de vestir y calzado fue el que menos aumentó en la medición sin estacionalidad, con una suba de `r cuadro1[11,7]` por ciento,**  seguido del índice de precios de Transporte, cuyo aumento en la medición sin estacionalidad fue de `r cuadro1[15,7]`  (Figura 2).<br>

```{r cuadrito_incidencias_mensual, include=FALSE}
arrange(cuadro2[5:16,],desc(Nacional))
```

**La contribución más importante al dato del nivel general del IPCse de `r meses[mes_t,1]` la hizo el índice de precios de la división Alimentos y bebidas no alcohólicas explicando alrededor de `r round(100*cuadro2[5,8]/cuadro2[2,8],0)` por ciento del `r cuadro1[2,7]` por ciento de inflación.** En la región del Gran Buenos Aires, esta división explicó un 27.5 por ciento de la tasa de inflación, mientras que, en el resto de las regiones, contribuyó a explicar entre 25.6 y 28.6 por ciento. Durante `r meses[mes_t,1]`, el segundo índice con la contribución a la tasa de inflación mensual más relevante fue el de Restaurantes y hoteles, explicando un 14.3 por ciento de la tasa de inflación mensual. La región en donde tomó la menor relevancia fue la región del GBA, explicando 20 por ciento, mientras que en la región Cuyo alcanzó casi el 15 por ciento. Vivienda, agua, electricidad, gas y otros combustibles explicó casi 32 por ciento de la tasa de inflación mensual en la región Noreste. Estas tres divisiones explican más de la mitad de la tasa de inflación nacional mensual sin estacionalidad. (Cuadro 2).<br>
