...
En `r meses[mes_t,1]` los factores estacionales contribuyeron a que la medición oficial de la inflación fuera `r text1` que sin estacionalidad, `r fg3t[1,2]` por ciento versus `r fg3t[1,3]` por ciento. Esta diferencia de `r abs(dif_indec_ipcse)` puntos porcentuales en la inflación con estacionalidad de `r meses[mes_t,1]` se explica fundamentalmente por el comportamiento de las divisiones de Alimentos y bebidas no alcohólicas, Prendas de vestir y calzado y Recreación y cultura,  donde los factores estacionales hicieron que la medición oficial fuera 0.7, 0.6 y 0.4 puntos porcentuales más alta, respectivamente (Figura 2). Durante los últimos 12 meses, el índice aumentó un `r cuadro1[2,8]` por ciento antes del ajuste estacional (datos oficiales de INDEC). <br>

```{r, include=FALSE}
fg2_wide %>%  arrange(desc(Diferencia))
```

**Durante `r meses[mes_t,1]`, todas las regiones registraron una tasa de inflación oficial mayor a la medición sin estacionalidad, con diferencias entre 0.03 y 0.40 puntos porcentuales.** La mayor diferencia se dio en la región de Patagonia, donde la tasa de inflación oficial fue 0.4 puntos porcentuales más alta que la medición sin estacionalidad, mientras que la menor diferencia ocurrió en la región Cuyo  (Figura 1).<br>

```{r, include=FALSE}
fg1t_wide %>%  arrange(desc(Diferencia))
```

**El índice de precios de Vivienda, agua, electricidad, gas y otros combustibles fue el que más aumentó en el mes, con un incremento de `r cuadro1[12,7]`. En segundo lugar en los aumentos está el índice de precios sin estacionalidad de Restaurantes y hoteles, cuyo incremento fue de `r cuadro1[19,7]`.** En general, como ocurrió en el mes anterior, los mayores aumentos del mes de `r meses[mes_t,1]` estuvieron ligados a divisiones con precios regulados. En un segundo escalón se ubican los aumentos en los índices de precios de Educación, Prendas de vestir y calzado y Salud con aumentos de `r cuadro1[18,7]`, `r cuadro1[11,7]` y `r cuadro1[14,7]`, respectivamente  (Cuadro 1).<br>

```{r, include=FALSE}
fg2_wide %>%  arrange(desc(IPCse))
```

**El índice de precios de Alimentos y bebidas no alcohólicas fue el que menos aumentó en la medición sin estacionalidad, con una suba de `r cuadro1[9,7]` por ciento, seguido del índice de precios de Transporte, cuyo aumento en la medición sin estacionalidad fue de `r cuadro1[15,7]` por ciento** (Figura 2). <br>

```{r cuadrito_incidencias_mensual, include=FALSE}
arrange(cuadro2[5:16,],desc(Nacional))
```

**La contribución más importante al dato del nivel general del IPCse de `r meses[mes_t,1]` la hizo el índice de precios de la división Vivienda, agua, electricidad, gas y otros combustibles explicando alrededor de `r round(100*cuadro2[8,8]/cuadro2[2,8],1)` por ciento del `r cuadro1[2,7]` por ciento de inflación.** La incidencia de este índice fue alta en todas las regiones del país, explicando entre 17.4 y 29.8 por ciento de las tasas de inflación regionales, alcanzando un mayor peso relativo en las regiones Noreste, Cuyo y Patagonia. En estas tres regiones, además de la región Pampeana, fue la división que más contribuyó a la tasa de inflación mensual, mientras que en GBA y Noroeste fue la división con la segunda mayor contribución. Durante `r meses[mes_t,1]`, la segunda contribución más grande a la tasa de inflación mensual fue la del índice de Restaurantes y hoteles, explicando un 16.9 por ciento de la tasa de inflación mensual. Esta división tuvo la mayor incidencia en la inflación en las regiones GBA y Noroeste, explicando 19.1 y 20.9 por ciento de la medición sin estacionalidad respectivamente. En las regiones Cuyo y Pampeana, Restaurantes y hoteles explicó 17.4 y 15 por ciento de la cifra de inflación sin estacionalidad, mientras que en las regiones Noreste y Patagonia esta división sólo explicó 11.2 y 11.9 por ciento de la inflación sin estacionalidad, respectivamente (Cuadro 2). La tercera división con mayor contribución fue Prendas de vestir y calzado, aunque empatada con la división Salud. Estas tres divisiones explican prácticamente la mitad de la inflación del meses de `r meses[mes_t,1]`. Este mes presenta la particularidad de que la incidencia de Alimentos y bebidas no alcohólicas en Cuyo fue negativa sugiriendo que la variación del nivel general de esa región hubiera caído 0.2 puntos porcentuales si sólo hubieran cambiado los precios de esa categoría, suponiendo que los precios del resto de la canasta se mantuvieron constantes. <br>
...
**En la comparación interanual, `r meses[mes_t,1]` de `r year_t` contra `r meses[mes_t,1]` de `r year_t - 1`, antes del ajuste estacional, el 50 por ciento del aumento de `r cuadro1[2,8]` por ciento está explicado por los índices de precios de tres de las divisiones de la canasta: Alimentos y bebidas no alcohólicas, Transporte, y Vivienda, agua, electricidad, gas y otros combustibles.** Restaurantes y Hoteles, con una incidencia interanual de `r sprintf("%.1f", cuadro2[32,8])` puntos porcentuales, y Salud, con `r sprintf("%.1f",cuadro2[27,8])` puntos porcentuales, son las divisiones que le siguen en la explicación del aumento interanual del nivel general de precios (Cuadro 2).<br>

```{r cuadrito_subyacente, include=FALSE}
cuadro1[21:27,]
```

**En `r meses[mes_t,1]`, la inflación subyacente, definida como el cambio porcentual en el nivel general del IPCse menos el efecto de los índices de precios de las divisiones de la canasta que incluyen bienes y servicios regulados o con alto componente impositivo, fue de `r cuadro1[21,7]` por ciento después de haber aumentado un `r cuadro1[21,6]` por ciento en `r meses_anterior[mes_t,1]`.** La inflación subyacente del mes de `r meses[mes_t,1]` tuvo un comportamiento relativamente homogéneo entre regiones. En las todas se registra un descenso de la tasa de inflación subyacente mensual con respecto al mes anterior. La región Patagonia registró la menor reducción, seguida de GBA, pero en el resto de las regiones  la inflación subyacente se redujo entre 0.25 y 0.50 puntos porcentuales. Todas las regiones presentan una inflación subyacente menor a la medición del nivel general sin estacionalidad, con diferencias entre 0.25 y 0.50 puntos porcentuales, a excepción de la región Cuyo, con una diferencia de 0.70. En términos interanuales, la inflación subyacente en el país se ubica en `r sprintf("%.1f", cuadro1[21,8])` por ciento en `r meses[mes_t,1]` (Cuadro 1).
...
**En `r meses[mes_t,1]`, la diferencia entre la tasa de inflación mensual que enfrentaron los hogares de menores ingresos y la de mayores ingresos fue de `r sprintf("%.2f", abs(cuadro3se[1,3]-cuadro3se[5,3]))` puntos porcentuales en detrimento de los hogares de mayores ingresos.** Esta diferencia se explica por los menores aumentos en los índice de precios de Alimentos y bebidas no alcohólicas, donde los hogares de mayores ingresos (quintil 5) concentran una menor proporción de su gasto en comparación con la proporción del gasto total que le dedican los hogares de menores ingresos (quintil 1).
...
El Cuadro 4 muestra el impacto diferencial de la inflación por quintiles de ingreso per cápita familiar para los datos oficiales. **Los hogares de menores ingresos, quintil 1, enfrentaron en `r meses[mes_t,1]` una inflación de `r sprintf("%.2f",cuadro4[1,3])` por ciento comparada con una inflación de `r sprintf("%.2f",cuadro4[5,3])` por ciento para los hogares del 20 por ciento de mayores ingresos** (ver Figura 5). Esta diferencia se debe al aumento de los precios de las divisiones de Transporte, Restaurantes y hoteles y Salud, que impactan en mayor medida en los hogares de mayores ingresos (quintil 5), combinado con menores aumentos de los índices de precios de las divisiones de Alimentos y bebidas no alcohólicas y Prendas de vestir y calzado, que impactan más en los hogares de menores ingresos (quintil 5).
