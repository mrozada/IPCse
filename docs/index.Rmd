...
### Inflación sin estacionalidad
  ...
  **El Índice de Precios al Consumidor nacional sin estacionalidad (IPCse) aumentó un `r cuadro1[2,7]` por ciento en `r meses[mes_t,1]`, luego de haber aumentado un `r cuadro1[2,6]` por ciento en `r meses[mes_tm1,1]`.** <br>
  ...
  En `r meses[mes_t,1]` los factores estacionales contribuyeron a que la medición oficial de la inflación fuera `r text1` que sin estacionalidad, `r fg3t[1,2]` por ciento versus `r fg3t[1,3]` por ciento. Esta diferencia de `r abs(dif_indec_ipcse)` puntos porcentuales en la inflación sin estacionalidad de `r meses[mes_t,1]` se explica fundamentalmente por el comportamiento de la división de Educación donde los factores estacionales hicieron que la medición oficial fuera 2.6 puntos porcentuales más baja (6.6 por ciento versus 9.2) (Figura 2). Durante los últimos 12 meses, el índice aumentó un `r cuadro1[2,8]` por ciento antes del ajuste estacional (datos oficiales de INDEC). <br>
  
  
  **Durante `r meses[mes_t,1]`, el comportamiento estacional fue mayormente parejo. En todas las regiones, la estacionalidad hizo que la medición oficial fuera de entre 0.1 y 0.4 puntos porcentuales menor al índice sin estacionalidad.** El impacto fue mayor en la región de Noreste, donde la inflación ajustada por estacionalidad fue de 7.0 por ciento, mientras que la medición oficial fue de 6.6 por ciento. En el Gran Buenos Aires el impacto fue de 0.2 puntos porcentuales, con la medición oficial registrando 5.8 por ciento, mientras que la cifra para la medición sin estacionalidad fue de 6.0 por ciento. (Figura 1).<br>
  
  **El índice de precios de comunicación fue, con diferencia, el que más aumentó en el mes con un incremento de `r cuadro1[16,7]` por ciento.** En un segundo escalón estuvieron los incrementos en educación, con `r cuadro1[18,7]` por ciento y los de salud, con `r cuadro1[14,7]` (Cuadro 1).<br>
  
  **Los índices de precios de prendas de vestir y calzado y bebidas alcohólicas y tabaco son los que menos aumentaron en la medición sin estacionalidad, con aumentos de `r cuadro1[10,7]` por ciento.** El índice de prendas de vestir y calzado muestra una diferencia positiva de 0.2 puntos porcentuales entre la medición sin estacionalidad y la medición de INDEC, mientras que para bebidas alcohólicas y tabaco la diferencia tiene el signo contrario, es menor para la medición de INDEC, y es de sólo 0.1 puntos porcentuales. El índice de alimentos y bebidas no alcohólicas también se encuentra dentro de los que menos aumentaron, con una suba de  `r cuadro1[9,7]` por ciento, superando a la medición oficial de 4.4 por ciento (Figura 2).<br>
  
  **La contribución más importante al dato del nivel general del IPCse de `r meses[mes_t,1]` la hizo el índice de precios de la división alimentos y bebidas no alcohólicas explicando alrededor de `r round(100*cuadro2[5,8]/cuadro2[2,8],0)` por ciento del `r cuadro1[2,7]` por ciento de inflación.** Las otras divisiones con incidencias importantes en el aumento del nivel general de precios sin estacionalidad fueron las de salud y transporte, explicando 0.7 puntos porcentuales cada una. El índice de precios de la división alimentos y bebidas no alcohólicas contribuyó a explicar el 37 y 31 por ciento de la inflación sin estacionalidad en las regiones noreste y noroeste, respectivamente. En la región del Gran Buenos Aires la división de alimentos y bebidas no alcohólicas explicó el 17 por ciento del incremento en el IPCse regional, mientras que en las regiones restantes explicó alrededor de 26% (Cuadro 2).<br>
  
  **En la comparación interanual, `r meses[mes_t,1]` de `r year_t` contra `r meses[mes_t,1]` de `r year_t - 1` antes del ajuste estacional, más del 50 por ciento del aumento de `r cuadro1[2,8]` por ciento está explicado por los índices de precios de tres de las divisiones de la canasta: alimentos y bebidas no alcohólicas, prendas de vestir y calzado, y restaurantes y hoteles.**  Transporte con una incidencia interanual de 11.8 puntos porcentuales y Salud con 9.3 puntos porcentuales son las divisiones que le siguen en la explicación del aumento interanual del nivel general de precios sin estacionalidad (Cuadro 2).<br> 
  
  **En `r meses[mes_t,1]`, la inflación subyacente, definida como el cambio porcentual en el nivel general del IPCse menos el efecto de los índices de precios de las divisiones de la canasta que incluyen bienes y servicios regulados o con alto componente impositivo, fue de `r cuadro1[21,7]` por ciento después de haber aumentado un `r cuadro1[21,6]` por ciento en `r meses[mes_t-1,1]`**  El valor de `r meses[mes_t,1]` es una combinación de valores relativamente mayores en las regiones Noreste y Patagonia y valores relativamente más bajos en GBA y Cuyo. En términos interanuales la inflación subyacente en el país ya superó el 120 por ciento en `r meses[mes_t,1]` (Cuadro 1).
...
```{r cuadro1, echo=FALSE, message=FALSE}
  ...
  cuadro1 %>%   
    ...
    kable_styling() %>%
    #add_header_above(c("Año"=1, "2022"=1,"2023"=5, setNames(1,text_mes_yoy))) %>%
...
### Inflación para hogares de mayores y menores ingresos
  ...
  **En `r meses[mes_t,1]`, la tasa de inflación mensual que enfrentaron los hogares de menores ingresos fue de `r sprintf("%.1f",cuadro3se[1,3])` por ciento mientras que para los hogares del 20 por ciento de mayores ingresos la inflación fue de `r sprintf("%.1f",cuadro3se[5,3])` por ciento.** Comparando contra el mismo mes del año anterior fue de `r sprintf("%.1f",cuadro3se[1,4])` por ciento para el 20 por ciento con menores y mayores ingresos (Cuadro 3).
  
 <!-- de `r sprintf("%.1f",cuadro3se[5,4])` por ciento para el quintil de mayores ingresos  -->
  
  **En `r meses[mes_t,1]`, la diferencia entre la tasa de inflación mensual que enfrentaron los hogares de menores ingresos y la de mayores ingresos fue de 0.4 puntos porcentuales.** Esta diferencia se explica casi totalmente por el menor aumento relativo en el índice de precios de la división de alimentos y bebidas no alcohólicas donde los hogares de menores ingresos (quintil 1) concentran una mayor proporción de su gasto en comparación con la proporción del gasto total que le dedican los hogares de mayores ingresos (quintil 5).      
